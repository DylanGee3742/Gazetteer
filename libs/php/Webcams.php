...
//"https://api.windy.com/api/webcams/v2/list/country=" . $_REQUEST['iso_a2'] . "?show=webcams:player,location&key=SECRET";

$url = "https://api.windy.com/api/webcams/v2/list/country=GB?show=webcams:player,location&key=SECRET";
...
if ($cURLERROR) {
    ...
    $webcam = json_decode($result, true);
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        if (isset($webcam['error'])) {

            $output['status']['code'] = $webcam['error']['code'];
            ...
            foreach ($webcam['result']['webcams'] as $item) {

                $cam['city'] = $item['location']['city'];
                $cam['latitude'] = $item['location']['latitude'];
                $cam['longitude'] = $item['location']['longitude'];
                $cam['player'] = $item['player'];

            array_push($finalResult, $cam);
